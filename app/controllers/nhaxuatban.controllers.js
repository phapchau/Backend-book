...
exports.create = async (req, res, next ) => {
    ...
    try {
        const contactService = new NhaXuatBanService(MongoDB.client);
        // Tạo một tài liệu mới với các trường name, username và password
        const exitstingUser =await contactService.findOne({tennxb: reg.body.tennxb});
        if (exitstingUser) {
            res.send("nha xuat ban already");
        }
        else {
            const document = await contactService.create({
                manxb: reg.body.manxb,
                tennxb: reg.body.tennxb,
